...
interface ArmCareSummaryDataPoint {
  date: string;
  value: number | null;
...
interface ArmCareSummaryData {
  latestExam: any;
  history: {
    armScore: ArmCareSummaryDataPoint[];
    totalStrength: ArmCareSummaryDataPoint[];
    totalStrengthPost: ArmCareSummaryDataPoint[];
  };
  stats: {
    totalExams: number;
...
export function ArmCareSummaryView({ playerId, data }: Props) {
  ...
  const strengthChartData = history.totalStrength
    ...
    .filter((entry) => entry.pre !== null || entry.post !== null);
