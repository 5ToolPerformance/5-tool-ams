import { LessonSelect } from "@ams/db/types/database";
import {
  LessonCounts,
  LessonWithCoachAndUser,
  WeeklyChartData,
} from "@ams/db/types/lessons";
 ...
 */
   ...
   */
export async function countLessons(lessons: LessonSelect[]) {
    try {
   ...
   * @throws Error if there is an error with the database query
   */
export async function countLessonsByLessonType(lessons: LessonWithCoachAndUser[]) {
    try {
   ...
   * @throws Error if there is an error with the database query
   */
export async function countPlayers(lessons: LessonWithCoachAndUser[]) {
    try {
...
export async function getPlayerIdsFromLessons(lessons: LessonWithCoachAndUser[]) {
    try {
      const playerIds = lessons.map((lesson) => lesson.lesson.playerId);
      const uniquePlayerIds = [...new Set(playerIds)];
...
export function countLessonsByDateRange(lessons: LessonSelect[]): LessonCounts {
    try {
...
export function getLessonsByCurrentWeek(lessons: LessonSelect[]): WeeklyChartData[] {
