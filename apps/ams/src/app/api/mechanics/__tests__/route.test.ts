...
const { POST } = require("@/app/api/mechanics/route");
...
describe("POST /api/mechanics", () => {
  beforeEach(() => {
    ...
    (fetchInternalApi as jest.Mock).mockResolvedValue(
      createApiResponse({ id: "mechanic-1", name: "Balance" }, { status: 201 })
    );
  });
  ...
  function createJsonRequest(body: unknown) {
    ...
    return {
      url: "http://ams.test/api/mechanics",
      method: "POST",
      headers: {
        get: (name: string) =>
  ...
  it("forwards mutation method, content type, and body to the signed API client", async () => {
    const requestBody = {
      name: "Balance",
      type: "hitting",
      tags: ["setup"],
  ...
  it("passes through API error responses", async () => {
    (fetchInternalApi as jest.Mock).mockResolvedValue(
      createApiResponse({ error: "Failed to create mechanic" }, { status: 400 })
    );
