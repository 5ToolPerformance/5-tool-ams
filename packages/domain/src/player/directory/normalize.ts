...
import type { PlayerDirectoryPositionInput, PlayerDirectoryQueryInput } from "./input";
...
function resolvePrimaryAndSecondaryPositions(
  rows: PlayerDirectoryPositionInput[],
...
export function normalizePlayersForDirectory(
  queryData: PlayerDirectoryQueryInput
): PlayerDirectoryItem[] {
  const positionsByPlayer = new Map<string, PlayerDirectoryPositionInput[]>();
