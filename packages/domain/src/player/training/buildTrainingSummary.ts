...
import type { LessonDiscipline, LessonSummary } from "../shared/lesson-summary";
...
export function buildTrainingSummary(
  ...
  const last14: LessonSummary[] = [];
  const last30: LessonSummary[] = [];

  const disciplineCounts: Record<LessonDiscipline, number> = {
    strength: 0,
    hitting: 0,
    pitching: 0,
    fielding: 0,
  ...
  const disciplineBreakdown = Object.entries(disciplineCounts)
    ...
    .map(([discipline, count]) => ({
      discipline: discipline as LessonDiscipline,
      count,
    }))
