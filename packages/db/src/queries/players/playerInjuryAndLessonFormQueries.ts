...
import type { PlayerInjuryInsert } from "../../types/database";
...
export async function getPlayerInformationByIdScoped(playerId: string, facilityId: string) {
    try {
      const [player] = await db
        .select()
        .from(playerInformation)
   ...
   */
export async function getPlayerInjuriesByPlayerId(playerId: string) {
    try {
   ...
   */
export async function createPlayerInjury(data: PlayerInjuryInsert) {
    if (!data.playerId) {
   ...
   */
export async function updatePlayerInjury(id: string, data: Partial<PlayerInjuryInsert>) {
    if (!id) {
   ...
   */
export async function listPlayersForLessonForm() {
    try {
      const players = await db
        .select({
...
export async function listPlayersForLessonFormScoped(facilityId: string) {
    try {
      const players = await db
        .select({
          id: playerInformation.id,
