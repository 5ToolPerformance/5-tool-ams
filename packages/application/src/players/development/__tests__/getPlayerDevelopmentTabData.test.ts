...
jest.mock("@ams/db/queries/evaluations/getEvaluationsForPlayer", () => ({
  getEvaluationsForPlayer: jest.fn(),
}));

jest.mock("@ams/db/queries/evaluations/getEvaluationById", () => ({
  getEvaluationById: jest.fn(),
}));

jest.mock(
  "@ams/db/queries/development-plans/getDevelopmentPlansForPlayers",
  () => ({
    getDevelopmentPlansForPlayer: jest.fn(),
  })
);

jest.mock("@ams/db/queries/config/listActiveDisciplines", () => ({
  listActiveDisciplines: jest.fn(),
}));

jest.mock("@ams/db/queries/routines/getRoutinesForPlayer", () => ({
  getRoutinesForPlayer: jest.fn(),
}));

jest.mock("@ams/db/queries/routines/listUniversalRoutines", () => ({
