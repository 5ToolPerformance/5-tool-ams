...
jest.mock(
  "@ams/db/queries/development-plans/getActiveDevelopmentPlanForPlayerDiscipline",
  () => ({
...
jest.mock("@ams/db/queries/players/getDisciplinesByIds", () => ({
  getDisciplinesByIds: jest.fn(),
}));

jest.mock("@ams/db/queries/players/PlayerHeader", () => ({
  getPlayerHeader: jest.fn(),
}));

jest.mock("@ams/db/queries/routines/getRoutinesForDevelopmentPlan", () => ({
