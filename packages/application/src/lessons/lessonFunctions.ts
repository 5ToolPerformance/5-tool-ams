...
import { AssessmentType } from "@ams/contracts/types/assessments";
import {
  ArmCareInsert,
...
} from "@ams/db/types/database";
import { LessonCreateData, LessonType } from "@ams/db/types/lessons";
import {
  getArmCareAssessmentById,
  getForcePlateAssessmentById,
  getHittingAssessmentById,
  getPitchingAssessmentById,
  getSmfaAssessmentById,
  getTrueStrengthAssessmentById,
} from "@/application/assessments/assessmentFunctions";
 ...
 */
   ...
   */
export async function createLesson(data: LessonCreateData) {
    try {
...
export async function getLessonsByCoach(coachId: string) {
    try {
...
export function validateLessonData(data: LessonCreateData): string[] {
    const errors: string[] = [];
...
export async function deleteLessonById(id: string) {
   ...
   * @throws Error if there is an error with the database query
   */
export async function updateLesson(id: string, data: Partial<LessonCreateData>) {
    try {
   ...
   * @throws Error if there is an error with the database query
   */
export async function getLessonsByCoachWithJoin(coachId: string) {
    try {
      return await db
        .select({
          lesson: lesson,
   ...
   * @throws Error if there is an error with the database query
   */
export async function getLessonsByPlayerWithJoin(playerId: string) {
    try {
      return await db
        .select({
          lesson: lesson,
...
export async function getLastLessonsByPlayer(playerId: string, count: number) {
    try {
      return await db
        .select()
        .from(lesson)
...
export async function getWriteupsByPlayer(playerId: string) {
    try {
      return await db
        .select()
        .from(writeups)
   ...
   */
export async function getNumberOfLessonsByPlayer(userId: string) {
    try {
      const lessons = await db
        .select({
          lesson: lesson,
   ...
   * @throws Error if there is an error with the database query
   */
export async function getLessonsByPlayerWithJoinAndAssessments(userId: string) {
    try {
      return await db
        .select({
          lesson: lesson,
   ...
   * @throws Error if there is an error with the database query
   */
export async function getLessonAssessmentById(id: string, type: string) {
    if (type === "force_plate") {
      return await getForcePlateAssessmentById(id);
    } else if (type === "true_strength") {
      return await getTrueStrengthAssessmentById(id);
    } else if (type === "arm_care") {
      return await getArmCareAssessmentById(id);
    } else if (type === "smfa") {
      return await getSmfaAssessmentById(id);
    } else if (type === "hitting") {
      return await getHittingAssessmentById(id);
    } else if (type === "pitching") {
      return await getPitchingAssessmentById(id);
    } else {
