...
import { getArmCareConfig } from "../config";
...
class ArmCareService extends BaseExternalService<
  ArmCareAPIResponse,
  ArmCareExamRecord,
  ArmCareConfig
> {
...
export async function syncArmCare(source: "cron" | "manual" = "manual") {
  const armcare = new ArmCareService();
  return armcare.sync(source);
}
